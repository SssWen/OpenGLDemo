...
int main()
{
    ...
    //glGenRenderbuffers(1, &captureRBO); // 没用到depthbuffer 所以这里先去掉depthBuffer绑定到Framebuffer

    glBindFramebuffer(GL_FRAMEBUFFER, captureFBO);
    //glBindRenderbuffer(GL_RENDERBUFFER, captureRBO);
    //glRenderbufferStorage(GL_RENDERBUFFER, GL_DEPTH_COMPONENT24, 512, 512);
	//glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, captureRBO);
	// 将Renderbuffer(渲染缓冲)对象 附加到 FRAMEBUFFER(帧缓冲)的深度和模版附件上
    //glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, GL_RENDERBUFFER, captureRBO);
...
#pragma region 将矩形天空盒--->6个方向的CubeMap，贴图为  newport_loft.hdr - > envCubemap.hdr
    ...
    for (unsigned int i = 0; i < 6; ++i) // 遍历6个方向,生成6个方向的CubeMap贴图,最后得到的贴图为envCubemap
    {
...
#pragma region 采样cubemap,生成漫反射的卷积cubemap,辐照度cubemap,也是6个方向，envCubeMap.hdr -> irradianceMap.hrd
    ...
    for (unsigned int i = 0; i < 6; ++i)
    {
		...
		// glFramebufferTexture2D(,tex[所有的渲染都写入该纹理]) 
		// 把一个纹理附加到FrameBuffer上,意味着所有的渲染操作都会写入这个纹理
...
#pragma region 渲染采样辐照度cubemap的漫反射BRDF,修改粗糙度，金属度，生成若干个球体进行对比
...
#pragma region 光源，渲染4个小球模拟光源,这个应该放在上面初始化光源位置 
...
#pragma region 最后渲染光源，减少overdrall.

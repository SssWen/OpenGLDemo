...
int main()
{
    ...
    glBindTexture(GL_TEXTURE_CUBE_MAP, envCubemap); // 这里生成6个方向的CubeMap贴图
    ...
    for (unsigned int i = 0; i < 6; ++i) // 遍历6个方向,生成6个方向的CubeMap贴图,最后得到的贴图为envCubemap
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, envCubemap, 0);//FrameBuffer渲染到贴图
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);

        renderCube(); // 将2维的HDR图片,转换为 一个立方体贴图,使用CubeMapShader,
    }
